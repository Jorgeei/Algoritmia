...
#opcion 3 se muestra la informacion de los estudiantes.
        if opcion=='3':
            print('\n\t|Modificar nota|\n')
            print('{:<13}{:<13}{:<13}{:<13}'.format('Codigo','Nombre','Nota 1','Nota 2'))
            for i,j in zip(llave,valor):
...
#se pide por pantalla el codigo del estudiante para cambiar una o ambas notas definitivas.
            ...
            if nueva_nota==1:
                nota_1=float(input('\nDigite una nueva Nota 1 {}: '.format(registro[codigo][0])))
                registro[codigo][1]=nota_1
                continue
            if nueva_nota==2:
                nota_2=float(input('\nDigite una nueva Nota 2 {}: '.format(registro[codigo][0])))
                registro[codigo][2]=nota_2
                continue
            if nueva_nota==3:
                nota_1=float(input('\nDigite una nueva Nota 1 de {}: '.format(registro[codigo][0])))
                nota_2=float(input('Digite una nueva Nota 2 de {}: '.format(registro[codigo][0])))
                registro[codigo][1]=nota_1
                registro[codigo][2]=nota_2
                continue
...
#se pide por patalla el codigo del estudiante y observar su promedio final.
            ...
            print('{:<13}{:<13}'.format(registro[nota_definitiva][0],(registro[codigo][1] + registro[nota_definitiva][2])/2))
#programa principal.
registro=dict()
llave=registro.keys()
